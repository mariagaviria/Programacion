...
with open('coordenadas-PRY.csv')as File:
...
def funpry():
    ...
    for i in red.keys():
        if (i!='PRY'):
            for j in red[i].keys():
                if (j=='PRY'):
                    imp+=red[i][j]                    
                    if (i=='ARG' or i=='BOL' or i=='BRA' or i=='CHL' or i=='COL' or i=='ECU' or i=='PER' or i=='VEN' or i=='URY'):
            ...
            for j in red[i].keys(): 
                exp+=red[i][j]                
                if (j=='ARG' or j=='BOL' or j=='BRA' or j=='CHL' or j=='COL' or j=='ECU' or j=='PER' or j=='VEN' or j=='URY'):
                    if (red[i][j]>0 and red[i][j]<=2000000):
    ...
    expn=exp-imp
    canvas.create_text(coordenadas['PRY'][0], 563, text=('Exportacion= ',exp, ' de dolares'), font=('Times New Roman',10))
    canvas.create_text(coordenadas['PRY'][0], 598, text=('Importacion= ',imp,' de dolares'), font=('Times New Roman',10))
    canvas.create_text(coordenadas['PRY'][0], 633, text=('Exportacion neta= ',expn,' de dolares'), font=('Times New Roman',10))
