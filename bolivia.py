...
with open('coordenadas-BOL.csv')as File:
...
def funbol():
    ...
    for i in red.keys():
        if (i!='BOL'):
            for j in red[i].keys():
                if (j=='BOL'):
                    imp+=red[i][j]                    
                    if (i=='ARG' or i=='VEN' or i=='BRA' or i=='CHL' or i=='COL' or i=='ECU' or i=='PER' or i=='PRY' or i=='URY'):
            ...
            for j in red[i].keys(): 
                exp+=red[i][j]                
                if (j=='ARG' or j=='VEN' or j=='BRA' or j=='CHL' or j=='COL' or j=='ECU' or j=='PER' or j=='PRY' or j=='URY'):
                    if (red[i][j]>0 and red[i][j]<=2000000):
    ...
    expn=exp-imp
    canvas.create_text(coordenadas['BOL'][0], 563, text=('Exportacion= ',exp, ' de dolares'), font=('Times New Roman',10))
    canvas.create_text(coordenadas['BOL'][0], 598, text=('Importacion= ',imp,' de dolares'), font=('Times New Roman',10))
    canvas.create_text(coordenadas['BOL'][0], 633, text=('Exportacion neta= ',expn,' de dolares'), font=('Times New Roman',10))
